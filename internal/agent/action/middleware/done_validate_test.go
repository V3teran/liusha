...
func TestDoneValidate_NotDone(t *testing.T) {
	...
	mw := DoneValidate(v)
	called := false
	exec := mw(func(_ context.Context, _ string, _ json.RawMessage) (action.Result, error) {
		called = true
		return action.Result{Output: []byte(`{}`)}, nil
	})
...
func TestDoneValidate_DoneOK(t *testing.T) {
	...
	exec := mw(func(_ context.Context, _ string, _ json.RawMessage) (action.Result, error) {
		called = true
		return action.Result{Done: true, Output: []byte(`{}`)}, nil
	})
...
func TestDoneValidate_NilValidatorSafe(t *testing.T) {
	...
	mw := DoneValidate(nil)
	exec := mw(func(_ context.Context, _ string, _ json.RawMessage) (action.Result, error) {
		return action.Result{Done: true}, nil
	})
