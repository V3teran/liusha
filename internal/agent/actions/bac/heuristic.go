...
func (a *HeuristicCheck) Execute(_ context.Context, args json.RawMessage) (action.Result, error) {
	var in struct {
		Rules []string `json:"rules"`
	}
	if len(args) > 0 {
		if err := json.Unmarshal(args, &in); err != nil {
			return action.Result{}, fmt.Errorf("解析 heuristic_check 参数失败: %w", err)
		}
	}
...
func marshalHeuristic(out heuristicOutput) (action.Result, error) {
	enc, err := json.Marshal(out)
	if err != nil {
		return action.Result{}, fmt.Errorf("序列化 heuristic_check 输出失败: %w", err)
	}
	summary := "heuristic_check skip=false"
	if out.Skip {
		summary = fmt.Sprintf("heuristic_check skip=true rule=%s", out.HitRule)
	}
	return action.Result{Output: enc, Summary: summary}, nil
}
