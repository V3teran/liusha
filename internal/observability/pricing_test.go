...
func TestEstimate_DeepSeek(t *testing.T) {
	got := testPricing.Estimate("deepseek", "deepseek-chat", llm.Usage{
		InTokens:  1_000_000,
		OutTokens: 1_000_000,
	})
	want := 0.27 + 1.10
...
func TestEstimate_Anthropic_CacheDiscount(t *testing.T) {
	got := testPricing.Estimate("anthropic", "claude-sonnet-4-6", llm.Usage{
		InTokens:     1_000_000,
...
func TestEstimate_Haiku(t *testing.T) {
	got := testPricing.Estimate("anthropic", "claude-haiku-4-5", llm.Usage{
		InTokens:  1_000_000,
		OutTokens: 1_000_000,
	})
	want := 1.00 + 5.00
...
func TestEstimate_UnknownReturnsZero(t *testing.T) {
	if v := testPricing.Estimate("unknown", "x", llm.Usage{InTokens: 100}); v != 0 {
...
func TestEstimate_ZeroUsage(t *testing.T) {
	if v := testPricing.Estimate("deepseek", "deepseek-chat", llm.Usage{}); v != 0 {
