...
//
// 参数 agentHunterID / ownerType / ownerID 是 registry-scope（per hunter run）：
...
func Record(store *toolinvocation.Store, agentHunterID, ownerType, ownerID string) toolfx.Interceptor {
	return func(next toolfx.ActionExecutor) toolfx.ActionExecutor {
		return func(ctx context.Context, name string, args json.RawMessage) (toolfx.Result, error) {
			...
			if store == nil || agentHunterID == "" || ownerID == "" {
