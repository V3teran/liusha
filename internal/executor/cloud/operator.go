// Package cloud 实现云环境渗透领域的 Executor
package cloud
...
// AgentFunc 按招法执行云渗透 agent
type AgentFunc func(ctx context.Context, m planner.Move) error

// Executor 是 cloud 域的 cognition.Executor：
// 执行 AWS/Azure/GCP/K8s 等云环境的渗透测试
type Executor struct {
	taskID   string
	target   string // 云环境目标（账户ID/订阅ID/项目ID/集群名）
	findings FindingLister
	run      AgentFunc
}

// NewExecutor 构造 cloud Executor
...
// 快照 → 执行云渗透 agent → 收割新 finding → 转 Attempt
func (e *Executor) Execute(ctx context.Context, m planner.Move) ([]verifier.Attempt, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cloud.Executor: 快照运行前 finding 失败: %w", err)
	}
	...
	if err := e.run(ctx, m); err != nil {
		return nil, fmt.Errorf("cloud.Executor: 云渗透 agent 执行失败: %w", err)
	}

	// 收割新 finding
	after, err := e.findings.ListByTaskAndHost(ctx, e.taskID, e.target, 0)
	if err != nil {
		return nil, fmt.Errorf("cloud.Executor: 收割 finding 失败: %w", err)
	}
	...
	for _, f := range after {
		...
		if err != nil {
			return nil, fmt.Errorf("cloud.Executor: finding %s 转 Attempt 失败: %w", f.ID, err)
		}
		if ok {
