...
func (p *Profile) Onboard(_ context.Context, in domain.BriefInput) ([]explorationgraph.TargetRef, error) {
	hosts := extractHosts(in.Brief)
	if len(hosts) == 0 {
		return nil, fmt.Errorf("executor.Onboard: brief 中未发现 http(s) 目标")
	}
	refs := make([]explorationgraph.TargetRef, 0, len(hosts))
	for _, h := range hosts {
		refs = append(refs, explorationgraph.TargetRef{
			Domain:  "web",
			RefKind: "host",
			Locator: h,
		})
