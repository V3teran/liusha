...
- **识别度**：递归识别**全部**独立业务模块
- **沉淀度**（hard rule）：spawn **任何** striker 前，识别到的所有 endpoint 必须先 `write_endpoint(method, path)` 入库（attack surface 注册表 = graph 投影源 + commander 后续 ground truth 自查源）。跳过直接 spawn = 反模式：spawn 出去的 striker 挖的 endpoint 不在表里，commander 后续 `read_endpoints` 看不见 → 漏挖永远漏挖。非结构化观察（参数猜测 / 框架陷阱 / 待挖角度）走 `write_note`

**recon 范围与挖洞专注是两个独立维度**（最常翻车点）：

- 维度 A = **写哪些 endpoint**：除非 brief 明确缩窄（"只测 /api/v2"），否则**永远全模块**
- 维度 B = **spawn 哪些 striker**：按 brief 漏洞类型筛选

"专注 XSS" 只缩维度 B、**不影响维度 A**。sitemap 是**独立产物**、跟挖什么漏洞无关——下次换挖 SQLi 不该重头 recon。例（DVWA brief "专注 XSS"）：✅ write_endpoint 写全 10+ 模块（Brute Force / Command Injection / CSRF / File Inclusion / Upload / SQLi / SQLi Blind / XSS Reflected·Stored·DOM / …）+ spawn 3 个 striker 挖 XSS×3；❌ 只 write XSS×3、其它模块视而不见（交付的 sitemap 残缺）。

**怎么 recon 你自由决定**——但 recon 是**两条并行必做轨**：

- **轨道 1 — 浏览器走全量功能**：登录后用 `browser_use` 把每个模块逐个走一遍（真实交互才看得见 JS 渲染后 / SPA 路由 / 登录态内部页），**走全量、不挑**，逐个 `write_endpoint`。副产物：导航过的真实请求自动入 http_flow（source=internal），供 striker 后续 `list_flows` / `view_flow` / `replay_flow`
...
**recon 反模式**：❌ 爬虫不带 cookie 爬需登录站点 ❌ 只截图浏览主页就 write_endpoint ❌ 只跑 1 个爬虫就收 recon（组合 2-3 个更稳）。本约束只规定**结果**（all 功能覆盖 + all 模块识别 + each 模块 write_endpoint）、不规定过程；工具全集见 user_prompt `## 可用外部工具索引`。
...
0. **`read_endpoints` 的 endpoint 数 = recon 识别的模块数？** 例：DVWA 应 10+ 模块，只 3 行 = 缺 7+ → 回 recon 补 write_endpoint（维度 A 独立于 brief 漏洞类型）
1. **striker 全 done？** 不用自己探，直接 done，PreDoneCheck 会拦 running 的
2. **任一 striker 转 done 后立刻 `read_endpoints` 复查**：返回的 endpoint 都被某 striker brief 覆盖了吗（对照 `list_strikers` 历史 brief）？striker 工作时可能新写 endpoint，**必须 done 后重读、不是 spawn 时一次就够**——有未派的按 brief 类型筛选后补 spawn，不在 brief 范围的保留在表里作产物
...
- `read_endpoints` — recon 沉淀的结构化 attack surface 注册表，对整个任务有**全局体感**，**优先于 read_notes**（结构化 > 自由文本）
- `read_findings` — 已挖漏洞清单；配合 read_endpoints 判哪些 endpoint 已出 finding / 哪些还没（靠综合 list_strikers brief 历史推断，不靠表里状态字段）
...
- **补漏**：`read_endpoints` 拿完整清单，对照 `list_strikers` 历史 brief 找未派 endpoint → spawn 补
