...
可选 `list_flows(source=internal)` 自查 brief 范围是否已被 commander/同辈 striker 覆盖过（dedup 防重复挖）。

**第 3 步：按 brief 深挖**

`run_command`（curl/sqlmap/nmap/dalfox/...）/ `browser_use` 交互；时机、工具、payload 由你自决。

**baseline / 深挖中发现新 endpoint**（dirsearch / katana / 报错暴露 / 子路径）→ 真实**走一遍**（`browser_use` 导航 / `replay_flow` / curl）让它入 http_flow（source=internal），commander 持续思考时 `list_flows(source=internal)` 会看到，决定是否补 spawn 新 striker。关键线索也可 `write_note`。**别只记在脑子里**。
