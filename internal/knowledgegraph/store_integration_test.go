...
//   - Lead→Result 晋升链：RecordVerification → confirmed 节点 → enables 边
...
func TestWorldModel_PromotionFlow(t *testing.T) {
	...
	verID, err := store.RecordVerification(ctx, worldmodel.Verification{
		...
		Evaluation:   json.RawMessage(`{"poc":"' OR 1=1--"}`),
		DurationMs: 1200,
	})
	if err != nil {
	...
	// 4) 晋升：落 confirmed 的 result 节点，verified_by 指向取证记录。
	result, err := store.UpsertNode(ctx, worldmodel.Node{
		TaskID:     taskID,
		Kind:       worldmodel.KindResult,
	...
	if err != nil {
		t.Fatalf("落 result: %v", err)
	}
	if result.Confidence != worldmodel.ConfConfirmed {
		t.Errorf("result 应 confirmed, got %s", result.Confidence)
	}
	if result.VerifiedBy == nil || *result.VerifiedBy != verID {
		t.Errorf("result.VerifiedBy 应指向 %s, got %v", verID, result.VerifiedBy)
	}

	// 5) confidence 单向：对已 confirmed 的 result 再写 assumed，不得降级。
	...
	// 幂等连边：重复不报错、不重复插。
	if err := store.LinkEdge(ctx, worldmodel.Edge{
		TaskID: taskID, Rel: worldmodel.RelOn, Src: result.ID, Dst: asset.ID,
	}); err != nil {
		t.Fatalf("幂等连边: %v", err)
	}
	...
	if len(nodes) != 2 {
		t.Fatalf("应有 2 个节点(asset+result), got %d", len(nodes))
	}
	...
	if edges[0].Rel != worldmodel.RelOn || edges[0].Src != result.ID || edges[0].Dst != asset.ID {
