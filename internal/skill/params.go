...
// BuilderParams 是 agent agent 的启动参数（与 SKILL.md Loader 同包）。
//
// 两种模式：
//   - passive: runner ingestor 拉到 traffic 后填 TrafficID/URL/Method + Request*/Response*，
...
type BuilderParams struct {
	...
	Traffic []traffic.ProxyTraffic

	// Active 模式独有：用户自然语言任务简报（含目标 URL/凭据/测试方向等所有信息）。
	Brief string

	// CliTools 是本操作员的外置 CLI 工具集（tools.yaml 名字），来自 agent.CliTools。
	// buildToolingCatalog 经 manifest.FilterByNames 严格白名单过滤：
	// 空 = 空集（不装配任何外部工具）；非空 = 只渲染白名单内工具（操作员专精，只给它这几把刀）。
	CliTools []string
