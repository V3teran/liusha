...
// ListRelationsByEngagement 列出本 engagement 下所有 finding 之间的 relation。
//
// JOIN finding 表过滤——只返回 from/to 都在本 engagement 内的边。
func (s *Store) ListRelationsByEngagement(ctx context.Context, engagementID string) ([]Relation, error) {
	rows, err := s.pool.Query(ctx, `
		...
		WHERE f1.engagement_id = $1 AND f2.engagement_id = $1
