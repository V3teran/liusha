...
//   - host 段保留原样（含端口；v1.1 方案 C 后全链路含端口，多端口部署据此区分）
...
//	→ bac.horizontal_priv_esc:vulnapp:GET:/api/order/:id
//
//	sqli.error_based:api.example.com:8080:GET:/api/v1/orders/123
//	→ sqli.error_based:api.example.com:8080:GET:/api/v1/orders/:id   （host 保留端口 + path 数字段模板化）
//
...
func NormalizeDedupKey(rawKey string) string {
	...
	// host 段保留端口（v1.1 方案 C 之后全链路含端口）：同一主机的不同端口属于
	// 不同部署/不同应用实例，dedup 不应合并。
