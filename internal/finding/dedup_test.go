...
func TestNormalizeDedupKey(t *testing.T) {
	cases := []struct {
		...
		{
			name: "host 含端口（IP:port）保留端口（v1.1 方案 C）",
			in:   "sqli.error_based:203.0.113.5:8080:GET:/api/v1/products:id",
			want: "sqli.error_based:203.0.113.5:8080:GET:/api/v1/products:id",
		},
		{
			name: "host 含端口（域名:port）保留端口（v1.1 方案 C）",
			in:   "sqli.error_based:api.example.com:8443:POST:/api/login",
			want: "sqli.error_based:api.example.com:8443:POST:/api/login",
		},
		{
			name: "host 不带端口保持原样",
			in:   "sqli.error_based:api.example.com:GET:/api/v1/products:id",
			want: "sqli.error_based:api.example.com:GET:/api/v1/products:id",
		},
		{
