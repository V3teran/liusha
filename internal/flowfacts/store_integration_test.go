...
// TestStore_Append_Roundtrip 写一条 flow_facts 后用 ListByEngagement 拉回，
...
func TestStore_Append_Roundtrip(t *testing.T) {
	...
	s := NewStore(pool)
	got, err := s.Append(ctx, Facts{
		EngagementID:        e.ID,
		FlowID:              fid,
		Operation:           "read",
		ResourceScope:       ResourceScopePrivate,
		ParamLocations:      json.RawMessage(`["json","query"]`),
		CarriesAuth:         true,
	...
	if string(d.ParamLocations) != `["json", "query"]` &&
		string(d.ParamLocations) != `["json","query"]` {
		t.Fatalf("param_locations: %s", d.ParamLocations)
	}
}
