...
func FormatSection(grouped map[Priority]map[Category][]Insight) string {
	...
	b.WriteString("## 洞察黑板（assignment 共享）\n")
...
// renderInsight 渲染单条洞察
func renderInsight(b *strings.Builder, i Insight) {
	// 格式：- [置信度图标] 摘要 (来源: task-xxx) [tag1, tag2]
	icon := confidenceIcon(i.Confidence)
	fmt.Fprintf(b, "- %s %s", icon, i.Summary)

	// 来源
	if i.SourceTaskID != "" {
		taskShort := i.SourceTaskID
		if len(taskShort) > 8 {
	...
	// 详细内容（缩进显示）
	if i.Body != "" {
		lines := strings.Split(strings.TrimSpace(i.Body), "\n")
