...
// - hypothesis: executor 产出的待验证假设（有 confidence）
// - evidence: verifier 产出的验证证据（有 outcome）
// - finding: 确认的安全发现/漏洞（有 confidence=verified）
//
// 关系边界：
// - GENERATES: action → hypothesis（动作生成假设）
// - CONFIRMS: evidence → finding（证据确认发现）
// - REFUTES: evidence → hypothesis（证据反驳假设）
// - ENABLES: finding → action（发现使能新动作）
...
const (
	...
	KindHypothesis NodeKind = "hypothesis" // 待验证假设（executor 产出）
	KindEvidence   NodeKind = "evidence"   // 验证证据（verifier 产出）
	KindFinding    NodeKind = "finding"    // 确认的发现（verified）
)
...
// Confidence 是 hypothesis/finding 的置信度
...
const (
	...
	SourceVerifier SourceType = "verifier" // verifier 创建
...
type Node struct {
	...
	DependsOn     []string    `json:"depends_on,omitempty"`     // 依赖的其他 action ID
	BlockedReason *string     `json:"blocked_reason,omitempty"` // 阻塞原因

	// hypothesis/finding 专用字段
...
// IsHypothesis 判断节点是否是 hypothesis
func (n *Node) IsHypothesis() bool {
	return n.Kind == KindHypothesis
}

// IsFinding 判断节点是否是 finding
func (n *Node) IsFinding() bool {
	return n.Kind == KindFinding
}

// IsEvidence 判断节点是否是 evidence
func (n *Node) IsEvidence() bool {
	return n.Kind == KindEvidence
}
...
// IsVerified 判断 hypothesis/finding 是否已验证
