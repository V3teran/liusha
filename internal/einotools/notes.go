...
func BuildReadNotes(store NoteStore, ownerID, host, hunterID string) (tool.BaseTool, error) {
	return utils.InferTool(
		"read_notes",
		"读取本次扫描（owner）共享笔记板——与同 host 其他 agent task（tracker / commander / striker）共享的过程性事实。"+
...
func BuildWriteNote(store NoteStore, ownerID, host, hunterID string) (tool.BaseTool, error) {
	return utils.InferTool(
		"write_note",
		"写一条过程性事实到本 (owner + host) 信息黑板。passive 共享给同 host 后续 tracker；active 用作长任务 step 间外置记忆（防 ReAct 滑窗压缩丢早期决策）。owner 关闭即过期。"+
			"\n\n【必写】仅适合 note 的内容："+
