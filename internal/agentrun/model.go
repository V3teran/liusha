...
//   - striker：parent_id 指向父 agent_run.id；striker**不**入 asynq，
...
//     Registry 已丢，list_strikers 工具看不到崩溃前的子；active 父走 asynq
...
// CommanderID 空表示独立/根任务；非空时指向父 agent_run.id（subtask swarm）。
