...
func (s *Store) Create(ctx context.Context, p NewParams) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("insert operator run: %w", err)
	}
	return id, nil
}
...
func (s *Store) SetRunning(ctx context.Context, id string) error {
	...
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("operator run %s not in pending state", id)
	}
	return nil
}
...
func (s *Store) SetDone(ctx context.Context, id string, result json.RawMessage) error {
	...
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("operator run %s not in pending|running state", id)
	}
	return nil
}
...
func (s *Store) GetByID(ctx context.Context, id string) (Run, error) {
	...
	if err := scanRun(row, &r); err != nil {
		return Run{}, fmt.Errorf("get operator run %s: %w", id, err)
	}
	return r, nil
}
...
func (s *Store) ListByTask(ctx context.Context, taskID string, limit int) ([]Run, error) {
	rows, err := s.pool.Query(ctx,
		...
		taskID, limit)
	if err != nil {
		return nil, fmt.Errorf("list operator runs by task: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := scanRun(rows, &r); err != nil {
			return nil, fmt.Errorf("scan operator run: %w", err)
		}
		out = append(out, r)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate operator runs: %w", err)
	}
	return out, nil
}
