...
// system.go：把 config.yaml 的业务旋钮（react 会话压缩 / runtime 工具运行时 / proxy_filter
// 流量过滤规则）insert-only 首填进 system_setting（migration 0098）三行分组 KV。
//
// 迁移语义（对齐 hunter/scenario/llm 种子）：DB 是事实源，种子只填**缺失组**——
// 每组按 group_key 判存在（GetXxxRaw 绕缓存直穿 DB），缺行才写；已存在一律跳过，
// 绝不覆盖前端「系统配置」或运维在 DB 里的改动。三组彼此独立，各自判存。

// ImportSystem 把 cfg 里的 react/runtime/proxy_filter 三组旋钮 insert-only 首填进 DB。
// 每组独立判存（缺组才写），任一组已存在则跳过该组。
func ImportSystem(ctx context.Context, cfg config.Config, s *settingstore.Store) error {
	if err := importReactSettings(ctx, cfg, s); err != nil {
		return fmt.Errorf("import react settings: %w", err)
	}
...
// importReactSettings 缺 react 组才用 yaml 的 history_compact 值首填。
func importReactSettings(ctx context.Context, cfg config.Config, s *settingstore.Store) error {
	if _, err := s.GetReactRaw(ctx); err == nil {
		return nil // 已存在→跳过（insert-only）
	} else if !settingstore.IsNotFound(err) {
		return fmt.Errorf("查 react 组: %w", err)
	}
	hc := cfg.React.HistoryCompact
	if err := s.SaveReact(ctx, settingstore.ReactSettings{
		TriggerRatio:            hc.TriggerRatio,
		TrailingBudgetRatio:     hc.TrailingBudgetRatio,
		CompactorTimeoutSeconds: hc.CompactorTimeoutSeconds,
	}); err != nil {
		return fmt.Errorf("写 react 组: %w", err)
	}
	return nil
}
