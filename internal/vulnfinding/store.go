...
// vulnfindingLog 包级 logger，用于 best-effort 计数失败的 warn。
var vulnfindingLog = logx.New("vulnfinding")
...
//
// 用法：vulnfinding.NewStore(pool).WithCounter(engs)
...
const colsSelect = "id, engagement_id, task_id, source_flow_id, host, kind, severity, title, target, evidence, confidence, dedup_key, created_at"
...
//
// host_lesson 表通过 OnReSaved 钩子的 lesson.TouchByFinding 累积 hit_count，
...
func (s *Store) Save(ctx context.Context, f VulnFinding) (VulnFinding, bool, error) {
	...
	if err := tx.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM vuln_finding WHERE host=$1 AND dedup_key=$2)`,
		f.Host, f.DedupKey,
	...
	row := tx.QueryRow(ctx, `
		INSERT INTO vuln_finding
			(engagement_id, task_id, source_flow_id, host, kind, severity, title, target, evidence, confidence, dedup_key)
	...
	if s.engCounter != nil {
		if err := s.engCounter.IncrementFindingCount(context.Background(), saved.EngagementID, 1); err != nil {
			vulnfindingLog.Warn().Err(err).Str("engagement_id", saved.EngagementID).
...
func (s *Store) GetByID(ctx context.Context, id string) (VulnFinding, error) {
	row := s.pool.QueryRow(ctx,
		"SELECT "+colsSelect+" FROM vuln_finding WHERE id=$1", id)
	var f VulnFinding
...
// ListByEngagement 列出 engagement.scope_host 下每个 (host, dedup_key) 的**最新一行** finding。
//
...
func (s *Store) ListByEngagement(ctx context.Context, engagementID string) ([]VulnFinding, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM vuln_finding
		WHERE host = (SELECT scope_host FROM engagement WHERE id=$1)
...
func (s *Store) CountByEngagement(ctx context.Context, engagementID string) (int, error) {
	...
	if err := s.pool.QueryRow(ctx,
		`SELECT count(*) FROM vuln_finding WHERE engagement_id=$1`, engagementID,
	).Scan(&n); err != nil {
...
func (s *Store) HasDedupKey(ctx context.Context, engagementID, dedupKey string) (bool, error) {
	var exists bool
	if err := s.pool.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM vuln_finding WHERE engagement_id=$1 AND dedup_key=$2)`,
