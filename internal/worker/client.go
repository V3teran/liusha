...
// Client 是 asynq 生产者的薄封装，按 Role 路由到对应队列，并以 Payload.HunterID 实现幂等。
type Client struct {
	c *asynq.Client
}
...
func (c *Client) Enqueue(ctx context.Context, role Role, p Payload, opts ...asynq.Option) (string, string, error) {
	if p.HunterID == "" {
		return "", "", fmt.Errorf("worker: payload.HunterID 不能为空")
	}
	...
	taskOpts := append([]asynq.Option{
		...
		asynq.TaskID(p.HunterID),
	}, opts...)
	task := asynq.NewTask(TaskTypeRun, body, taskOpts...)

	info, err := c.c.EnqueueContext(ctx, task)
	if err != nil {
		return "", "", fmt.Errorf("worker: 入队 role=%s task=%s 失败: %w", role, p.HunterID, err)
	}
