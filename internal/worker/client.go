...
// Client 是 asynq 生产者的薄封装，按 Role 路由到对应队列，并以 Payload.ExecutorID 实现幂等。
type Client struct {
	c *asynq.Client
}
...
//
// 幂等：用 p.ExecutorID 作为 asynq 的 task ID，重复 Enqueue 同一 ExecutorID 会返回 asynq.ErrTaskIDConflict。
// opts 透传给 asynq.NewTask（如 asynq.MaxRetry(0) 用于 active planner禁止重试——
...
func (c *Client) Enqueue(ctx context.Context, role Role, p Payload, opts ...asynq.Option) (string, string, error) {
	if p.ExecutorID == "" {
		return "", "", fmt.Errorf("worker: payload.ExecutorID 不能为空")
	}
	...
	taskOpts := append([]asynq.Option{
		...
		asynq.TaskID(p.ExecutorID),
	}, opts...)
	task := asynq.NewTask(TaskTypeRun, body, taskOpts...)

	info, err := c.c.EnqueueContext(ctx, task)
	if err != nil {
		return "", "", fmt.Errorf("worker: 入队 role=%s task=%s 失败: %w", role, p.ExecutorID, err)
	}
