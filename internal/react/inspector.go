...
// StepRecord 是滑动窗喂给 Inspector 的最近 N 步记录。
//
// ObsSummary 字段保留：tool Action 可在 Result.Summary 主动设 ≤200 字摘要——多数 step 用它即可。
// FullObs 是工具完整输出（不截断）—— 仅最近 1 步（window 末尾）填，让 inspector 能看到
// 关键字（SUCCESS/vulnerable/uid=）防止摘要截断误判进度。零值时 buildInspectorPrompt 回退 ObsSummary。
...
// Inspector 在 ReAct 循环每 N 步触发一次，用最近窗口判断"该不该继续 / 怎么改向"。
//
// 由 T23.5 用 LLM 实现；T23 默认注入 NoopInspector 让循环行为与旧版兼容。
type Inspector interface {
	Evaluate(ctx context.Context, window []StepRecord) Verdict
}

// NoopInspector 永远返回 continue，便于在不需 Inspector 的场景注入。
type NoopInspector struct{}

// Evaluate 实现 Inspector。
func (NoopInspector) Evaluate(context.Context, []StepRecord) Verdict {
