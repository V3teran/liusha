...
- ❌ **同一工具连 3 次超时/失败仍微调 flag 重试**：sqlmap blind 慢、nuclei 模板不匹配等场景，**第 3 次失败必须 pivot 换手法**（如 sqlmap blind 失败 → curl 手动 boolean fuzz 或直接 write_finding 不 dump；nuclei 无命中 → 改 sqlmap/dalfox 等）。微调 flag 重试不是探索，是死循环
