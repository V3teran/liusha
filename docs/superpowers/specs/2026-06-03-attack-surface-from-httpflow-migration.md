...
- planner 靠**手动 `write_endpoint` 转写**recon 所见 → 易漏（这次就漏了参数维度）
...
### 归属（最硬的点，已解）
- 容器 per-run 共享（planner+exploitation），spawn 时绑 planner 的 agent_id（`launcher.Spawn(ctx, p.AgentID)`）
- CLI 流量按 owner 级归属：用容器绑定的 agent_id → ingestor `handleInternalSnap` 反查 owner（现有机制）
- agent_id 对 CLI 流量统一记 planner 的（owner 正确，graph 是 owner-scoped，够用）
...
**不复活 proxify internal listener**（v34 删的复杂度：Proxy-Auth/407/HTTPS session-stash/per-agent，容器 per-run 共享根本不需要 per-agent）。
...
2. 写 mitmproxy addon：每条 response → POST /internal/v1/flows/ingest（agent_id 从容器 env 取，owner 级归属）
3. **fuzz 去重在 addon 源头**：addon 内存维护 per-run 已见集合 `(method, templatize(path))`，重复直接不 POST（流量没入库就掐断；sqlmap 喷 1 万 → 1 条）+ 丢静态资源类
4. launcher 注入 `HTTP_PROXY/HTTPS_PROXY/ALL_PROXY` → 指向容器内 mitmproxy + 注入 agent_id env
...
9. 改 prompts：planner.md（recon 不再 write_endpoint，改"走全 → 流量自动成图"）、exploitation.md、BAC skill（read_endpoints→list_sitemap）
...
- 不复活 v34 删的 proxify internal listener（Proxy-Auth/407/per-agent 复杂度）；改 mitmproxy + 复用 ingest，owner 级归属，避开旧脆弱路径
