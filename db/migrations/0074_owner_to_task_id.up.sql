...
--   - 4 张共享表 agent / finding / llm_invocation / tool_invocation 去 owner_type+owner_id、
...
-- ── agent（历经 agent_run→agent_task→agent 重命名，owner 列自 0038 携带至今）
DROP INDEX IF EXISTS agent_run_owner_idx;
ALTER TABLE agent DROP COLUMN IF EXISTS owner_type;
ALTER TABLE agent DROP COLUMN IF EXISTS owner_id;
ALTER TABLE agent ADD COLUMN task_id uuid NOT NULL REFERENCES task(id) ON DELETE CASCADE;
CREATE INDEX agent_task_idx ON agent (task_id, created_at DESC);
...
-- 0055 把该列 rename 成 agent_id 时索引未随之改名，需先清掉旧索引再建同名新索引。
