-- 0050: agent_task.role 切换到 traffic-analysis/orchestrator/exploitation 三角色 enum + CHECK 约束。
--
-- 历史：0001 ~ 0049 期间 agent_task.role 都写 "hunter"（worker.RoleHunter 字面值），
-- 跟 asynq queue 路由 enum 耦合在同一字段。
-- v1.1: agent 角色拆 traffic-analysis（passive 单 agent）/ orchestrator（active 父）/ exploitation（active 子），
...
--      - owner_type='active_scan' + parent_id IS NULL        → 'orchestrator'
...
UPDATE agent_task SET role = 'orchestrator'
    WHERE role = 'hunter' AND owner_type = 'active_scan' AND parent_id IS NULL;

-- 2. 加 CHECK 约束（三值枚举）
ALTER TABLE agent_task
    ADD CONSTRAINT agent_task_role_check
    CHECK (role IN ('traffic-analysis', 'orchestrator', 'exploitation'));

COMMENT ON COLUMN agent_task.role IS
    'agent 角色枚举：traffic-analysis（passive 单 agent）/ orchestrator（active 父）/ exploitation（active 子）。跟 asynq queue 路由（worker.RoleAgent）解耦。';
