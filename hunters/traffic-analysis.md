...
拿到本 task 认领的**一批** mitmproxy 捕获流量，从线索追到漏洞落库。流量已绑 host，无需主动发现攻面。
...
- `write_finding` **必须带 `source_traffic_id`**：填本 finding 所依据的那条流量 id（来自 prompt 清单 / view_traffic）。一批多条流量，不锚定来源无从复核 —— 这是硬要求
